...
const Render = ()=>{
    ...
    useEffect(()=> {
        ...
        .then(response=>{
            const filteredWords = response.data.filter(word => word.lessonId === lessonId);
    ...
    const handleNext = ()=> {
        ...
        if (index+1<words.length) {
            ...
            navigate("/result", {state: {correct: count, noOfWords: words.length, results: result}})
        }
    }
    ...
    const countCorrectAnswer = () => {
        const {countWord, highlightedWord} = getHighlightedText();
        setResult(prev=>[...prev, highlightedWord])
    ...
    return (
        <div>
            ...
            {words[index] && (transcript?
            <p style={{color: "green"}}>{<div dangerouslySetInnerHTML={{ __html: getHighlightedText().highlightedWord }} />}</p>
